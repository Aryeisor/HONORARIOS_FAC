...
class OrtopediaRules(SpecialtyBase):
    ...
    contratos_coosalud = {"COOSALUD00225", "COOSALUD00125"}

    consulta_re = re.compile(r"\b(CONSULTA|INTERCONSULTA)\b", re.IGNORECASE)
    cuidados_re = re.compile(r"\b(CUIDAD(O|OS)?)\b", re.IGNORECASE)
    ...
    def detect_anulable_rows(self, ws_original, header_row, col_map):
        ...
        if col_estado is not None and col_saldo is not None:
            groups = {}
            ...
            for _, items in groups.items():
                ...
                tiene_anulado = "ANULADO" in estados
                tiene_base = ("SIN PAGOS" in estados) or ("PAGO TOTAL" in estados)
                saldo_sum = sum(it["saldo"] for it in items)

                if tiene_anulado and tiene_base and abs(saldo_sum) < 0.01:
                    for it in items:
    ...
    def process_row(
        ...
        is_coo = contrato in self.contratos_coosalud
        is_consulta = bool(self.consulta_re.search(proc_text))
        is_cuidados = bool(self.cuidados_re.search(proc_text))
        is_coo_proced = is_coo and (not is_consulta) and (not is_cuidados)
        ...
        obs = "NO COOSALUD"
        if is_coo and (is_consulta or is_cuidados):
            obs = "COOSALUD (consulta/cuidados) - sin cálculo"
        ...
        if is_coo_proced:
            ...
            if base_val != 0.0 and pct is not None:
                ...
                if abs(pct - 1.0) < 1e-9:
                    ...
                    obs = (obs + " | " if obs else "") + f"Porcentaje no esperado: {pct}"

            total = b100 + b75 + b70 + b60 + b50
            clin_value = total * payment_pct
            diff = saldo_val - clin_value
        ...
        nuevo_vlr_aut = None
        if is_coo_proced and idx_vlr_aut is not None:
            nuevo_vlr_aut = clin_value

        return {
            "destino": "pre",
            "calc_values": calc_values,
            "nuevo_vlr_aut": nuevo_vlr_aut,
            "is_coo_proced": is_coo_proced,
        }